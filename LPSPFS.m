function [feaIndx,W,obj] = LPSPFS(X,r1,r2,Y,Kmatrix,ksize,numFea) %% 
[num, dim] = size(X);
d = ones(dim,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
AA = myadjacency(X,ksize,Kmatrix);
DD = sum(AA,2);
L = spdiags(DD,0,speye(size(AA,1)))-AA;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if num<dim    
    ...
    while flag
        ...
        DX = D*X';
        %%% Notive trick!!!
        W = (DX*(eye(num)+ r2*num*L)*X + r1*num*eye(dim))\(DX*YL);     
        Xi = sqrt(sum(W.*W,2));
        d = 2*Xi;    
        XW = X*W -YL;
        obj(iter) = trace(XW*XW')/num + r2*trace(L*((X*W)*(X*W)')) + r1*sum(Xi);        
